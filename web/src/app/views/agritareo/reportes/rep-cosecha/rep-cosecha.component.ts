...
import { COSECHA } from '@app/common/constants/agritareo.constants';
...
@Component({
  selector: 'ns-rep-cosecha',
  templateUrl: './rep-cosecha.component.html',
  styleUrls: ['./rep-cosecha.component.scss']
})
...
export class RepCosechaComponent extends AbstractList implements OnInit {
  ...
  setInitialData(i?: string, f?: string) {
    this.http.get('phytosanitary-report', { i, f }).subscribe(data => {
      ...
      this.http.get('report-cosecha-lote').subscribe(data => {
  ...
  downloadData(print = false) {
    ...
    this.http.post('download', { headers, file: print ? 'pdf' : this.fileOption, cid: 'COSECHA', r, e },
